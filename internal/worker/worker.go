...
func New(queueURL, redisAddr string, redisPassword string, redisTLS bool, exec *executor.Executor, workers int) (*Worker, error) {
...
func (w *Worker) handleMessage(ctx context.Context, msg types.Message) {
	...
	if execErr != nil {
		...
		resultMap := responseToMap(req, resp)
...
// Update the signature to accept the original request data as well
func responseToMap(req executor.Request, resp executor.Response) map[string]any {
    m := map[string]any{
        "status": resp.Status,
    }
    if resp.Build != nil {
    ...
    if len(resp.Tests) > 0 {
        ...
        for i, t := range resp.Tests {
            tests[i] = map[string]any{
                ...
                // ─── ADD THESE TWO NATIVE GO FIELDS HERE ───
                "stdin":           req.TestCases[i].Input,
                "expectedOutput":  req.TestCases[i].ExpectedOutput,
            }
        }
        m["tests"] = tests
    }
